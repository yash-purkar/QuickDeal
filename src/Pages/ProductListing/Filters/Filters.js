    ...
    <div>
      ...
      <div className='filters-box' style={{ display: isFilters ? "flex" : "none" }}>
        ...
        <div className='flex direction-column padding-bottom-1 bottom-margin-md bottom-border-1'>
          {
            ["Men", "Women", "Kids"].map(category => <label htmlFor={category} className='cursor-pointer' key={category}>
              <input type="checkbox" className='bottom-margin-md font-roboto' onChange={() => dispatch({ type: "FILTER_BY_CATEGORIES", payload: category })} checked={selectedCategories.includes(category)} /> <span className='display-inline-block bottom-margin-md'>{category}</span>
            </label>)
          }
        </div>
        ...
        <div className='flex direction-column padding-bottom-1 bottom-margin-md bottom-border-1'>
          ...
          {
            ["S", "M", "L", "XL", "XXL"].map(size => <label htmlFor="" className='cursor-pointer' key={size} >
              <input type="checkbox" className='bottom-margin-md font-roboto' checked={selectedSizes.includes(size)} onChange={() => dispatch({ type: "FILTER_BY_SIZE", payload: size })} /> <span className='display-inline-block bottom-margin-md'>{size}</span>
            </label>)
          }
        </div>
        ...
        <div className='flex direction-column bottom-border-1 padding-bottom-1'>
          ...
          {[4, 3, 2, 1].map(item => <label key={item} htmlFor="rating" name="rating" className='padding-bottom-0-4 cursor-pointer'>
            <input type="radio" name="rating" value={item} checked={rating === item} onChange={() => dispatch({ type: "FILTER_BY_RATING", payload: item })} />
        ...
        <label htmlFor="lowToHigh" className='cursor-pointer' >
          <input type="radio" name="sort" checked={sort === "LOW_TO_HIGH"} onChange={() => dispatch({ type: "SORT_BY_PRICE", payload: "LOW_TO_HIGH" })} />
          <span className='display-inline-block bottom-margin-md'>Low To High</span>
        </label>

        <label htmlFor="highToLow " className='cursor-pointer '>
          <input type="radio" name="sort" checked={sort === "HIGH_TO_LOW"} onChange={() => dispatch({ type: "SORT_BY_PRICE", payload: "HIGH_TO_LOW" })} />
          <span className='display-inline-block bottom-margin-md '>  High To Low</span>
        </label>

        <label htmlFor="RESET " className='cursor-pointer padding-bottom-1  bottom-border-1'>
          <input type="radio" name="sort" onChange={() => dispatch({ type: "SORT_BY_PRICE", payload: "RESET" })} checked={sort === "RESET"} />
