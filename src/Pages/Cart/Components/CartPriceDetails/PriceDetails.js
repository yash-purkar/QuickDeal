...
export const PriceDetails = ({ cartData }) => {
  ...
  // New price and old price to show discount
  const newPrice = cartData?.reduce(
    (acc, curr) => curr.newPrice * curr.qty + acc,
    0
  );
  const oldPrice = cartData?.reduce(
    (acc, curr) => curr.oldPrice * curr.qty + acc,
    0
  );
  ...
  // handle checkout navigation and set the details
    ...
    <div className="price-detail-card">
      ...
      <div>
        ...
        <div className="flex justify-between total-amt top-margin border-top-1 top-padding-08">
          ...
          <h5 className="font-1 sm-margin-bottom">
            ₹ {(newPrice - couponDiscount).toFixed()}
          </h5>
        </div>
        ...
        <div className="flex justify-between coupon-box ">
          ...
          <button
            ...
            onClick={() => setIsHideCouponBox(false)}
          >
            Apply
          </button>
        </div>
      ...
      <p className="font-1 sm-margin-bottom saved-price-info">
        You will save ₹ {(couponDiscount + oldPrice - newPrice).toFixed()} on
        this order
      </p>

      <button className="checkout-btn  cursor-pointer" onClick={handleCheckOut}>
        Checkout
      </button>

      {!isHideCouponBox && (
        <CouponBox setIsHideCouponBox={setIsHideCouponBox} />
      )}
    </div>
  );
};
