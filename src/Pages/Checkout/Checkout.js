...
export const Checkout = () => {
  ...
  const {
    orderState: { price, discount, couponDiscount, totalAmount },
    orderDispatch,
  } = OrderState();
  ...
  // cart items id to remove them from the cart after checkout
  const cartItemsId = cart?.reduce((acc, curr) => [...acc, curr._id], []);

  // It handle place older and opens razorpay modal
      ...
      {isOrderPlaced ? (
        <div>
          <div className="truck-gif-box  ">
            <img
              ...
              alt="gif"
              className="truck-gif top-margin-5 margin-bottom-1"
            />
          </div>
          <h1 className="text-center letter-spacing margin-bottom-1 order-succesfully">
            Order Placed Successfully
          </h1>
          <p className="letter-spacing text-center order-will-deliever">
            Your order will be delievered in 3 working days
          </p>
        </div>
      ) : (
        <div className="checkout-main-container flex justify-around  wrap">
          ...
          <div className="checkout-addresses ">
            <>
              {addresses.length > 0 ? (
                <>
                  {addresses?.map((addr) => {
                    ...
                    return (
                      <div
                        key={id}
                        className="single-checkout-address cursor-pointer"
                      ...
                      >
                        ...
                        <div className="left-padding-sm">
                          <p>
                            {street}, {cityName}, {state}, {postalCode}
                          </p>
                          <p>{country}</p>
                ...
                <h1 className="select-address-heading letter-spacing">
          ...
          <div className="checkout-container flex direction-column justify-between">
            <h3 className="text-uppercase border-top-1  padding-1 font-1 padding-left-0 text-center bottom-border-1 color-primary letter-spacing">
              order details
            </h3>
            ...
            {/*  */}
            <h3 className="text-uppercase border-top-1  margin-bottom-1 padding-1 font-1 padding-left-0 text-center bottom-border-1 color-primary letter-spacing">
              price details
            </h3>
            ...
            <button
              className="place-order-btn cursor-pointer"
